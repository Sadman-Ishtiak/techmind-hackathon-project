    ...
    <?php if ($result->num_rows > 0): ?>
        <?php while ($store = $result->fetch_assoc()): ?>
            <div class="bg-white rounded-lg shadow p-6 hover:shadow-lg transition duration-300">
                <h3 class="text-lg font-semibold mb-2"><?php echo htmlspecialchars($store['store_name']); ?></h3>
                <p class="text-gray-500 mb-4">Owner: <?php echo htmlspecialchars($store['owner_name']); ?></p>
                <a href="store_products.php?store_id=<?php echo $store['store_id']; ?>"
